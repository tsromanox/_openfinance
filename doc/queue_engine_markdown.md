...
Bucket4j é a biblioteca mais adequada para cenários multi-tenant com rate limiting distribuído. Ele implementa o algoritmo token bucket com suporte nativo a Redis via Redisson.

### Configuração Maven e Redis
...
### Serviço de rate limiting por instituição
...
CREATE TABLE institution_config (
    ...
    account_balance_tpm INTEGER NOT NULL DEFAULT 2500,
    transactions_tpm INTEGER NOT NULL DEFAULT 2000,
    credit_cards_tpm INTEGER NOT NULL DEFAULT 1500,
